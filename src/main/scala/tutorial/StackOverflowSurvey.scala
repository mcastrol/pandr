...
object StackOverFlowSurvey {
  ...
  def main(args: Array[String]) {
    ...
    val responses = dataFrameReader
      ...
      .csv("s3n:///shoesizeme-staging/uploads/plugin-OLD/2016-stack-overflow-survey-responses.csv")
    ...
    System.out.println("=== Print records where the response is from Spain ===")
    responseWithSelectedColumns.filter(responseWithSelectedColumns.col("country").===("Spain")).show()
